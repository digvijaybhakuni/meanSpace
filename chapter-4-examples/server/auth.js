  ...
  passport.use(new FacebookStrategy(
    ...
    function(accessToken, refreshToken, profile, done) {
      if (!profile.emails || !profile.emails.length) {
        console.log(profile);
        profile.emails = ['digvijaybhk@gmail.com'];
        //return done('No emails associated with this account! Sach M Bhai');
      }
      console.log(profile.emails);
      console.log(profile.emails[0].value);
      User.findOneAndUpdate(
        { 'data.oauth': profile._json.id },
        {
          $set: {
            'profile.username': 'digvijaybhk@gmail.com',//profile.emails[0].value,
            'profile.picture': 'http://graph.facebook.com/' +
              profile._json.id.toString() + '/picture?type=large'
          }
        },
